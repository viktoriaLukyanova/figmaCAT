...
(async() => {
    // Launching Chrome
    const browser = await puppeteer.launch({args: ['--no-sandbox', '--disable-setuid-sandbox'], headless: false, timeout: settings.launchTimeout});
    ...
    for(let i = 0; i < figmaFilesList.length; i++) {
        ...
        try {
            ...
            await page._client.send('Page.setDownloadBehavior', {behavior: 'allow', downloadPath: tmpDownloadDir.replaceAll('/', '\\').replace('.', 'C:\\Users\\Vikus\\finder')});
